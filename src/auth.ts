...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  try {
    const issuer = ApplicationConfig.keycloak.issuer;
    const url = `${issuer}/protocol/openid-connect/token`;

    const { data } = await axios.post(
      url,
      new URLSearchParams({
        client_id: ApplicationConfig.keycloak.clientId,
        client_secret: ApplicationConfig.keycloak.clientSecret,
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  providers: [
    Keycloak({
      clientId: ApplicationConfig.keycloak.clientId,
      clientSecret: ApplicationConfig.keycloak.clientSecret,
